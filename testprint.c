...
int _printf(const char *format, ...)
{
	...
	while (*format)
	{
		...
		{
			/* if next format is a null or nonformat character */
			...
			/* iterates through spaces after a % */
			while (*(format + 1) == ' ')
				format++;
			/* checks for last space && if the next one */
			if (*(format + 1) == '\0')
			{
				format++;
				continue;
			...
			/* if next format is valid && not valid */
			...
			{
				container = add_buff(container, args, format
				,0);
			}
			format++;
